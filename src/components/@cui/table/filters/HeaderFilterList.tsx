...
export interface HeaderColumnFilter {
  ...
  setColumnFilter?: React.Dispatch<
    React.SetStateAction<ColumnFilterType[]>
  >;
}
interface HeaderFilterListType
  extends HeaderColumnFilter {
  columnFilterField?: ColumnType[];
}

const HeaderFilterList: FC<
...
> = ({
  ...
  const showFilterColumns =
    columnFilterField?.map((item) => {
      const matchingFilter = columnFilter?.find(
        (filterItem) =>
          filterItem.id === item.filterId,
      );
      return {
        ...item,
        value: matchingFilter
          ? matchingFilter.value
          : "", // Provide default empty string if no matching filter
      };
    });
  return (
